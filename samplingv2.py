...
parser.add_argument('--train', dest='train', action='store_false', help='train the model')
parser.add_argument('--sample', dest='train', action='store_true', help='sample from the model')
...
def to_one_hot(s, ascii_steps, vocabulary):
    steplimit=3e3; s = s[:3e3] if len(s) > 3e3 else s # clip super-long strings
    seq = [vocabulary.find(char) + 1 for char in s]
    if len(seq) >= ascii_steps:
        seq = seq[:ascii_steps]
    else:
        seq = seq + [0]*(ascii_steps - len(seq))
    one_hot = np.zeros((ascii_steps,len(vocabulary)+1))
...
# 3.- Functions to help with sampling
...
def get_style_states(model, args):
    ...
    style_onehot = [to_one_hot(style_string, model.ascii_steps, args.vocabulary)]
        
    style_stroke = np.zeros((1, 1, 3), dtype=np.float32)
    style_kappa = np.zeros((1, args.num_mix_window, 1))
...
def sample(input_text, model, args):
    ...
    one_hot = [to_one_hot(input_text, model.ascii_steps, args.vocabulary)]         # convert input string to one-hot vector
    [c0, c1, c2, h0, h1, h2] = get_style_states(model, args) # get numpy zeros states for all three LSTMs
    kappa = np.zeros((1, args.num_mix_window, 1))   # attention mechanism's read head should start at index 0
	...
	eos = 1 if 0.35 < eos[0][0] else 0 # use 0.5 as arbitrary boundary
        ...
        finished = True if i > args.backprop_steps else False
...
def line_plot(strokes, title, figsize = (40,2), save_path='.'):
    import matplotlib as mpl
    mpl.use('Agg')
    import matplotlib.pyplot as plt
    plt.figure(figsize=figsize)
...
def sample_model(args, record=None):
	if args.text == '':
		strings = ['Por un Mexico mas libre', 'mas justo y mas prospero', 'contribuir a la formacion integral', \
			'excelencia academica dentro de la pluralidad de ideas', 'solo hay un itam'] # test strings
	else:
		strings = [args.text] 


	record = Record(args) if record is None else record # instantiate record, if None
	#record = Record(args)
	record.write("\nSAMPLING MODE...")
	record.write("loading data...")

	record.write("building model...")
	model = Model(args, record)
	...
	if load_was_success:
		for s in strings:
			...
			w_save_path = '{}figures/iter-{}-w-{}'.format(args.rec_dir, global_step, s[:10].replace(' ', '_'))
			g_save_path = '{}figures/iter-{}-g-{}'.format(args.rec_dir, global_step, s[:10].replace(' ', '_'))
			l_save_path = '{}figures/iter-{}-l-{}'.format(args.rec_dir, global_step, s[:10].replace(' ', '_'))
			...
			record.write( "kappas: \n{}".format(str(kappas[min(kappas.shape[0]-1, args.steps_per_char),:])) )
	else:
		record.write("load failed, sampling canceled")

	if True:
		tf.reset_default_graph()
		time.sleep(args.sleep_time)
		sample_model(args, record=record)
