...
class Model():
	def __init__(self, args, record):
		...
		self.num_mix_out = args.num_mix_out
		self.num_mix_window = args.num_mix_window
		self.batch_size = args.batch_size if self.train else 1 # training/sampling specific
		self.backprop_steps = args.backprop_steps if self.train else 1 # training/sampling specific
		self.vocabulary = args.vocabulary
		# training params
		self.dropout_prob = args.dropout_prob
		self.grad_clip = args.grad_clip
		# misc
		self.steps_per_char = args.steps_per_char
		...
		self.record.write('\tusing vocabulary{}'.format(self.vocabulary))
		self.char_vec_len = len(self.vocabulary) + 1 #plus one for <UNK> token
		self.ascii_steps = args.backprop_steps/args.steps_per_char


		# ----- build the basic recurrent network architecture
		cell_func = tf.contrib.rnn.LSTMCell # could be GRUCell or RNNCell
		#cell_func = tf.contrib.rnn.LSTMBlockCell # could be GRUCell or RNNCell
		self.cell0 = cell_func(args.hidden_size, state_is_tuple=True, initializer=self.graves_initializer)
		self.cell1 = cell_func(args.hidden_size, state_is_tuple=True, initializer=self.graves_initializer)
		self.cell2 = cell_func(args.hidden_size, state_is_tuple=True, initializer=self.graves_initializer)
		...
		self.input_data = tf.placeholder(dtype=tf.float32, shape=[None, self.backprop_steps, 3])
		self.target_data = tf.placeholder(dtype=tf.float32, shape=[None, self.backprop_steps, 3])
		...
		inputs = [tf.squeeze(input_, [1]) for input_ in tf.split(self.input_data, self.backprop_steps, 1)]
	...
	# ----- build the gaussian character window
		def get_window(alpha, beta, kappa, c):
			...
			# c -> [? x ascii_steps x vocabulary] and is a tf matrix
			ascii_steps = c.get_shape()[1].value #number of items in sequence
			phi = get_phi(ascii_steps, alpha, beta, kappa)
			window = tf.matmul(phi,c)
			window = tf.squeeze(window, [1]) # window ~ [?, vocabulary]
			return window, phi

		#get phi for all t,u (returns a [1 x backprop_steps] matrix) that defines the window
		def get_phi(ascii_steps, alpha, beta, kappa):
			# alpha, beta, kappa -> [?,num_mix_window,1] and each is a tf variable
		...
		def get_window_params(i, out_cell0, num_mix_window, prev_kappa, reuse=True):
			hidden = out_cell0.get_shape()[1]
			n_out = 3*num_mix_window
			...
			abk = tf.exp(tf.reshape(abk_hats, [-1, 3*num_mix_window,1])) # abk_hats ~ [?,n_out] = "alpha, beta, kappa hats"

			alpha, beta, kappa = tf.split(abk, 3, 1) # alpha_hat, etc ~ [?,num_mix_window]
			kappa = kappa + prev_kappa
			return alpha, beta, kappa # each ~ [?,num_mix_window,1]

		self.init_kappa = tf.placeholder(dtype=tf.float32, shape=[None, self.num_mix_window, 1]) 
		...
		for i in range(len(outs_cell0)):
			[alpha, beta, new_kappa] = get_window_params(i, outs_cell0[i], self.num_mix_window, prev_kappa, reuse=reuse)
	...
	# ----- start building the Mixture Density Network on top (start with a dense layer to predict the MDN params)
		n_out = 1 + self.num_mix_out * 6 # params = end_of_stroke + 6 parameters per Gaussian
		with tf.variable_scope('mdn_dense'):
			mdn_w = tf.get_variable("output_w", [self.hidden_size, n_out], initializer=self.graves_initializer)
			mdn_b = tf.get_variable("output_b", [n_out], initializer=self.graves_initializer)

		out_cell2 = tf.reshape(tf.concat(outs_cell2, 1), [-1, args.hidden_size]) #concat outputs for efficiency
	...
	# ----- build mixture density cap on top of second recurrent cell
		...
		self.cost = loss / (self.batch_size * self.backprop_steps)

		# ----- bring together all variables and prepare for training
		self.learning_rate = tf.Variable(0.0, trainable=False)
		self.decay_rmsprop = tf.Variable(0.0, trainable=False)
		self.momentum_rmsprop = tf.Variable(0.0, trainable=False)
		...
		if args.optimizer == 'adam':
			...
			self.optimizer = tf.train.RMSPropOptimizer(learning_rate=self.learning_rate, decay=self.decay_rmsprop, momentum=self.momentum_rmsprop)
		else:
	...
	def try_load_model(self, save_path):
		...
		try:
			...
			self.record.write("no saved model to load. starting new session")
			load_was_success = False
		else:
			self.record.write("loaded model: {}".format(load_path))
			self.saver = tf.train.Saver(tf.global_variables())
