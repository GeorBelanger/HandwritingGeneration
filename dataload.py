...
# class Dataloader to load handwriting data
class DataLoader():
    def __init__(self, args, record, limit = 500):
        self.data_dir = args.data_dir
        self.vocabulary = args.vocabulary
        self.batch_size = args.batch_size
        self.backprop_steps = args.backprop_steps
        ...
        if not (os.path.exists(data_file)) :
            self.record.write("\tcreating training data cpkl file from raw source")
    ...
    def preprocess(self, stroke_dir, ascii_dir, data_file):
        ...
        self.record.write("\tparsing dataset...")
...
#                 print 'processing '+stroke_file
                ...
                if len(ascii) > 10:
                    ...
                    self.record.write("\tline length was too short. line was: " + ascii)
        ...
        f.close()
        self.record.write("\tfinished parsing dataset. saved {} lines".format(len(strokes)))
    ...
    def load_preprocessed(self, data_file):
        ...
        # goes thru the list, and only keeps the text entries that have more than backprop_steps points
        self.stroke_data = []
        self.ascii_data = []
        ...
        for i in range(len(self.raw_stroke_data)):
            ...
            if len(data) > (self.backprop_steps+2):
        ...
        self.record.write("\tloaded dataset:")
        self.record.write("\t\t{} train individual data points".format(len(self.stroke_data)))
        self.record.write("\t\t{} valid individual data points".format(len(self.valid_stroke_data)))
        self.record.write("\t\t{} batches".format(self.num_batches))
    ...
    def validation_data(self):
        ...
        for i in range(self.batch_size):
            ...
            x_batch.append(np.copy(data[:self.backprop_steps]))
            y_batch.append(np.copy(data[1:self.backprop_steps+1]))
            ascii_list.append(self.valid_ascii_data[valid_ix])
        one_hots = [to_one_hot(s, self.ascii_steps, self.vocabulary) for s in ascii_list]
        return x_batch, y_batch, ascii_list, one_hots

    def next_batch(self):
        # returns a randomized, backprop_steps-sized portion of the training data
        x_batch = []
        y_batch = []
        ascii_list = []
        for i in xrange(self.batch_size):
            data = self.stroke_data[self.idx_perm[self.pointer]]
            idx = random.randint(0, len(data)-self.backprop_steps-2)
            x_batch.append(np.copy(data[:self.backprop_steps]))
            y_batch.append(np.copy(data[1:self.backprop_steps+1]))
